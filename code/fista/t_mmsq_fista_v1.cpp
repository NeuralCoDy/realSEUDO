...
testDiagonalMulti(const char *tname)
{
	...
	// per dimension makes the convergence much faster - in a single step.
	...
	Fista::MultiPosMatSquareMinimizer pl(k, b, lambda, 0.1);
...
testDiagonalMulti2(const char *tname)
{
	...
	// _other_ diagonals of the matrix, symmetric.
...
testAllMulti(const char *tname)
{
	...
	if (fabs(x_next[0] - 3.7) > 0.1
	|| fabs(x_next[1] - 2.4) > 0.1) {
		printf("%s: Too far from the expected result!\n", tname);
		return 1;
	}
	if (!stop) {
...
// Like Fista::MultiPosMatSquareMinimizer but with 0 >= x <= 256.
class PixelMinimizer : public Fista::MultiPosMatSquareMinimizer
{
public:
	...
	PixelMinimizer(
		...
		double eps)
		: Fista::MultiPosMatSquareMinimizer(k, b, lambda, eps)
	{
		upperBounded_ = true;
		upperBound_ = 256.;
	}
